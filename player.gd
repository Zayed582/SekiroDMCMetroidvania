...
var parry_burst_scene = preload("res://parryburst.tscn")

@onready var wall_check_left = $WallCheckLeft
@onready var wall_check_right = $WallCheckRight
var shock_blast_scene = preload("res://shock_blast.tscn")


@onready var afterimage_scene = preload("res://Afterimage.tscn")
...
var razor_bloom_petals_scene = preload("res://razor_bloom_petals.tscn")
var razor_bloom_explosion_scene = preload("res://razor_bloom_explosion.tscn")
@onready var echo_slash_scene = preload("res://echo_slash.tscn")
...
@onready var petal_projectile_scene = preload("res://razor_bloom_petals.tscn")
@onready var slash_explosion_scene = preload("res://razor_bloom_explosion.tscn")
...
func _physics_process(delta):
	...
	if is_dodging:
		...
		if not attacking and not is_parrying and not is_hurt and not recovering_from_knockback and not is_knockedback_sliding:
...
func _process(delta):
	...
	# 🔋 Charging logic
	if Input.is_action_pressed("attack") and not attacking and not is_parrying and not is_dodging and not is_hurt and not recovering_from_knockback and is_on_floor():
		if not is_charging:
			is_charging = true
			charge_timer = 0.0
		else:
...
func start_dodge(direction):
	...
	if overdrive_active and overdrive_moonveil and not get_tree().has_group("mirage"):
		...
		var mirage_scene = preload("res://moonveil_mirage.tscn")
